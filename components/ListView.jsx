...
export default function ListView({invoice, number}) {
    ...
    <tr className={`${ number != 0? '' : 'border-b-2'} border-neutral`}>
        ...
        <td className="p-2">{invoice.header}</td>
        <td className="p-2">{total} kr</td>
        <td className="p-2">
            <PaymentStatus status={invoice.paid} dueDate={invoice.dueDate}/>
        </td>
        <td className="p-2">
            <button className="flex items-center gap-1 bg-primary  w-3/4 rounded-full py-1 px-3 text-secondary font-semibold">
