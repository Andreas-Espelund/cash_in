...
export default function Navbar() {
  ...
  return (
    <div className="text-lg p-4 lg:px-8 items-center bg-primary flex justify-between">
        ...
        {session &&
          <button onClick={() => setDropdownOpen(e => !e)} className={`${dropdownOpen? 'bg-secondary' : 'bg-primary'} hover:bg-secondary transition-all duration-200 w-max text-white flex items-center justify-center rounded-full font-semibold p-2 gap-2`}>
            <AnimatePresence
              initial={true}
              wait
            >
            {dropdownOpen && 
            <motion.p
              className="hidden lg:block pl-1 h-8 overflow-hidden  text-start"
      ...
      >
          ...
          <Backdrop onClick={() => setDropdownOpen(false)} transparent={true}>
            ...
            <motion.div 
              className="flex flex-col fixed top-24 p-4 gap-4 rounded-2xl shadow-lg right-8 bg-white w-[90vw]  lg:w-fit"
              initial={{opacity:0}}  
              animate={{opacity:1}}  
              exit={{opacity:0}}
